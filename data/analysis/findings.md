...
- 高いのは**北部（大町・柏井町・曽谷・南大野・国府台）**。
  曽谷・大野・柏井・大町の 20 地区の平均は **28.8％**（地区名の**部分一致**で数えたので「南大野」も入る）
- 低いのは**南部の行徳・妙典エリア**。妙典／行徳／南行徳の 14 地区の平均は **12.8％**（こちらは**前方一致**。
  部分一致にすると「本行徳」などが入って 15 地区・13.2％になる）
...
- 公開されている AED は市内 **305 箇所**（うち座標が使えるのは **304 箇所**）
...
- 町丁字の重心から最寄りの**指定緊急避難場所**まで：中央値 **278m**・**最大でも 839m**（面的に整っている）
- 最寄りの**介護サービス事業所**まで：中央値 **566m**・**最大 1,653m**（塩浜4丁目）
- 高齢化率と距離の相関は弱い（避難場所 r≈0.01、介護事業所 r≈−0.31）。
  **介護事業所はむしろ高齢地区に寄って立地している**

**数字の確かめ方**: この節の値は `python3 data/scripts/verify_presentation_numbers.py` で数え直せる。
厳密には避難場所 277.79m・介護事業所 565.72m（2026-09-03 実測）。
**地区の数え方（前方一致か部分一致か）で地区数と平均が変わる**ので、引用するときは数え方を添える。
