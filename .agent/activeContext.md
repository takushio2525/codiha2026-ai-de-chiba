...
| 表示名 | 画面・ブラウザタイトル・README すべて **CHIZUBA**。ブラウザタイトルは `layout.tsx` の `title.template` が末尾に付けるので、**子ページは画面名だけ書く**。`compose.yaml` の `name: ichikawa-opendata-map` だけは据え置き（意図的） |
| ロゴ | 正本は `app/src/app/icon.svg`（開いた C ＋ 地点の点）。`favicon.ico` / `apple-icon.png` はそこから起こす。画面の中は `BrandMark.tsx` に同じ図形。**変えるなら両方** |
| 帯の段組み | **スマホ前提**。`AuthBar` は 〜639px＝ロゴとアイコンのみ / 640px〜＝＋ワードマーク / 768px〜＝＋ラベル。**段をまとめない**（同時に出る幅で余白が 10px まで詰まる） |
