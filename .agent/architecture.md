...
### ハザードタイルの疎通（2026-09-03 実測・市川市 z=14）
...
「土砂災害警戒区域 → 404（市川市は平地で該当区域が無い）」と書いてあったが**誤り**で、
市川市には **142 区域**が指定されている（**すべて急傾斜地の崩壊**なので、
試すタイルが違っていた）。内水も**水防法第14条の2 で指定・公表済み**だが、
**30.73 MB の PDF でしか出ておらず**タイルが無い。
出典と検証は `docs/design/requirements.md` §7-1 の訂正、
および `docs/presentation/審査基準_主張と根拠.md` §8-1。
