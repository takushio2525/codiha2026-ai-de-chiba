...
#slide(
  ...
  #grid(
    ...
    [
      ...
      #v(5pt)
      #head[① オープンデータの活用 ── 地図の上で可視化する]
      #v(2pt)
      #set text(size: 8.1pt)
      #text(weight: "bold")[千葉県・市川市から 13 データセット 17 ファイルを取得し，
      うち 4 種類を#text(fill: deep)[1 枚の地図に重ねて可視化]した．]
      重ねかたは #text(weight: "bold")[点・面・「今」]の 3 層．
      - #text(weight: "bold")[点で描く ── 市川市のオープンデータ（CC BY 4.0）]．
        避難場所 #text(weight: "bold")[123]・AED #text(weight: "bold")[304]・子育て施設 #text(weight: "bold")[388]・景観100選 #text(weight: "bold")[100] を色と形の違うアイコンで描き分け（色覚に配慮した配色），
        種類ごとに ON/OFF．押すと名称・所在地・対応する災害が出る
      - #text(weight: "bold")[面で敷く ── 国のハザードマップ]．
        洪水・高潮・津波の浸水深と土砂災害警戒区域の 4 種を#text(weight: "bold")[種類ごとに不透明度を変えて]重ねられ，
        凡例は深さと区域の種別で出し分ける
      - #text(weight: "bold")[その上に「今」を重ねる]．
        住民の投稿 3 種を色分けしたピンで同じ地図に置き，
        浸水報告には気象庁アメダスの雨量が入る．集めた投稿は CSV / GeoJSON で返す

      #v(2pt)
      #text(size: 8.1pt)[
        #text(weight: "bold")[課題の発見にも使った] — 分析で最寄りの避難場所まで#text(weight: "bold")[中央値 278 m]と分かり，
        「箱が足りない」を捨てた．背景地図は国土地理院，ハザードは国土交通省，雨量は気象庁で，
...
#slide(
  ...
  #let feat(no, title, body, how) = block(
    width: 100%,
    fill: white,
    inset: (x: 6pt, y: 5pt),
    radius: 3pt,
  ...
  #grid(
    ...
    shot(
      ...
      [洪水の浸水想定が重なった市川市の地図に，避難場所・AED・子育て施設と住民の投稿のピンが載る．
        左上は注意案内で，過去の浸水報告 5 件と気象庁の降水確率という#text(weight: "bold")[事実 2 つ]だけを並べている],
    ),
    shot(
