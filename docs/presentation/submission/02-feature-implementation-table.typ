...
#note[
  ...
  - #text(weight: "bold")[前提: 閲覧にログインは要らない．] 投稿・コメント・行政操作にだけログインが要る．
    #text(weight: "bold")[審査環境（認証キー未設定）ではデモログイン]（画面右上「ログイン」→ 表示名を入れるだけ）
    で全機能を試せる．Google ログインも実装してあるが，
    #text(weight: "bold")[クライアントシークレットは秘密情報なので提出物に同梱していない]．
    そのため #text(weight: "bold")[審査環境では検証できず，この表の行にしていない]
]
...
#table(
  ...
  ..row(
    ...
    [
      ...
      「投稿時の雨量 ◯ mm/h ／ 船橋アメダス（約 10.2 km）」が入る
    ],
  ),
