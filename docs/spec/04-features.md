...
### 3 つのレイヤー（`app/src/lib/hazards.ts` の `HAZARDS`）
...
- **タイルが 404 でも、その区域が無いとは限らない。** 実測で踏んだ例: 市川市は
  ...
  （すべて急傾斜地の崩壊で、そちらのタイルは 200 を返す）。内水（雨水出水）浸水想定も、
  指定・公表はされているがタイルが無い。経緯は `docs/design/requirements.md` §7-1 の訂正
